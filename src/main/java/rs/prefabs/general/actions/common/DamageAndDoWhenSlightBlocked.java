...
public class DamageAndDoWhenSlightBlocked extends AbstractPrefabGameAction {
    private AbstractGameAction.AttackEffect effect;
    private Consumer<AbstractCreature> dowhat;

    public DamageAndDoWhenSlightBlocked(AbstractCreature target, PrefabDmgInfo info, AbstractGameAction.AttackEffect effect, Consumer<AbstractCreature> dowhat) {
        this.target = target;
        this.info = info;
        this.effect = effect;
        this.dowhat = dowhat;
    ...
    public void update() {
        if (duration == startDuration) {
            ...
            if (target.lastDamageTaken < info.output && target.lastDamageTaken > 0)
