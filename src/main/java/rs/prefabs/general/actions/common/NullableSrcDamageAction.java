...
public class NullableSrcDamageAction extends AbstractPrefabGameAction {
    ...
    public NullableSrcDamageAction(AbstractCreature target, PrefabDmgInfo info, AttackEffect effect, boolean useBiteEffect) {
        this.goldAmount = 0;
        this.skipWait = false;
        this.muteSfx = false;
        this.info = info;
    ...
    public NullableSrcDamageAction(AbstractCreature target, PrefabDmgInfo info) { this(target, info, AttackEffect.NONE, false); }

    public NullableSrcDamageAction(AbstractCreature target, PrefabDmgInfo info, boolean superFast) {
        this(target, info, AttackEffect.NONE, false);
        this.skipWait = superFast;
    }

    public NullableSrcDamageAction(AbstractCreature target, PrefabDmgInfo info, AttackEffect effect) {
        this(target, info, effect, false);
        this.skipWait = false;
        this.muteSfx = false;
    }

    public NullableSrcDamageAction(AbstractCreature target, PrefabDmgInfo info, AttackEffect effect, boolean useBiteEffect, boolean superFast) {
        this(target, info, effect, useBiteEffect);
        this.skipWait = superFast;
    }

    public NullableSrcDamageAction(AbstractCreature target, PrefabDmgInfo info, AttackEffect effect, boolean useBiteEffect, boolean superFast, boolean muteSfx) {
        this(target, info, effect, useBiteEffect);
        this.skipWait = superFast;
    ...
    public void update() {
        if (this.duration == DURATION) {
            ...
            if (info.owner == null) {
                Debugger.Log(this, "dealing no source damage to " + target.name);
            }
