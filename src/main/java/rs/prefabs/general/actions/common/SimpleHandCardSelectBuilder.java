...
public class SimpleHandCardSelectBuilder extends AbstractPrefabGameAction {
    private String msg;
    ...
    public void update() {
        if (duration == startDuration) {
            ...
            if (removeList.size() == p.hand.group.size()) {
                Debugger.Log("No cards match the conditions.");
                isDone = true;
                return;
            }
                ...
                for (AbstractCard card : p.hand.group) {
                    if (!notToRemove.contains(card)) {
                        if (cm.manipulate(card, index)) {
                            Debugger.Log("Returning the selected card to hand: " + card.name);
            ...
            if (p.hand.group.size() > 1) {
                Debugger.Log("Opening hand card select screen...");
                AbstractDungeon.handCardSelectScreen.open(msg, amount, anyNumber, canPickZero, false, forUpgrade);
                tickDuration();
            } else if (p.hand.group.size() == 1) {
                Debugger.Log("Only one card left, manipulating to " + p.hand.getTopCard());
                if (cm.manipulate(p.hand.getTopCard(), 0)) {
                    Debugger.Log("Returning the selected card to hand: " + p.hand.getTopCard().name);
