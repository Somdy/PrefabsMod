...
import rs.prefabs.general.listeners.utils.CardPlayedEvent;
import rs.prefabs.general.listeners.utils.PlayCardEvent;
...
public class UseCardListener {
    private static List<CustomUnplayableLogic> unplayableList;
    private static List<CustomPlayCardEvent> playCardEventsList;
    private static List<CustomCardPlayedEvent> cardPlayedEventsList;
    public static List<AbstractCard> cardsPlayedLastTurn;
    ...
    private static void putPlayCardEventWithAbsenceCheck(String ID, boolean forcedReplace, int turnsLeft, PlayCardEvent event) {
        ...
        if (playevent == null) {
            ...
            if (!success)
                Debugger.Log(UseCardListener.class, "Failed to replace play card event: " + ID);
        }
    }
    ...
    public static void addNewCardPlayedEvent(String ID, boolean forcedReplace, int turnsLeft, CardPlayedEvent event) {
        ...
        if (playedevent == null) {
            ...
            if (!success)
                Debugger.Log(UseCardListener.class, "Failed to replace card played event: " + ID);
        }
    }
    ...
    private static void putLogicWithAbsenceCheck(AbstractCard card, int turns, Function3<AbstractCard, 
        ...
        if (logic == null) {
            ...
            if (!success)
                Debugger.Log(UseCardListener.class, "Failed to replace unplayable logic of card: " + card.name);
        }
    }
    ...
    public static boolean canCardPlay(AbstractCard card, AbstractPlayer p, AbstractMonster m) {
        if (unplayableList.stream().noneMatch(e -> e.identify(card))) {
            Debugger.Log(UseCardListener.class, card.name + " has no custom unplayable logic");
            return true;
        }
        CustomUnplayableLogic logic = null;
        for (CustomUnplayableLogic lg : unplayableList) {
        ...
        if (logic == null) {
            Debugger.Log(UseCardListener.class, "Unable to find " + card.name + "'s custom unplayable logic");
            return true;
        }
        int turnsLeft = logic.accept(card);
        if (turnsLeft > 0) {
            boolean canPlay = logic.find(card).invoke(card, p, m);
            Debugger.Log(UseCardListener.class, "Detected " + card.name + "'s custom unplayable logic: " + canPlay);
            return canPlay;
        } else {
            Debugger.Log(UseCardListener.class, card.name + " should've been removed from unplayable list since " + (-turnsLeft) + " ago.");
