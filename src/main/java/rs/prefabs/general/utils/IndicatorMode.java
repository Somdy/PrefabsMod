...
public class IndicatorMode implements GeneralUtils {
    ...
    public IndicatorMode(Object register) {
        this.register = register;
    }

    public static void register(IndicatorMode mode) {
        Debugger.Log("Registering new invoker...");
        invoker.add(mode);
    }
    ...
    public void active(@NotNull Vector2 start, Consumer<AbstractCreature> dowhat, Predicate<AbstractCreature> predicator,
                       Color pointerColor) {
        Debugger.Log("Invoking indicator at " + start.x + ", " + start.y);
        this.start = start;
        startX = start.x;
        startY = start.y;
        this.dowhat = dowhat;
    ...
    public void execute(@NotNull AbstractCreature t) {
        Debugger.Log("Executing final action at " + t.name);
        dowhat.accept(t);
        drawLine = false;
        clearPostUsed();
    }
    ...
    public static class DarkenInoptionalCrtPatch {
        private static final ShaderProgram grayShader = new ShaderProgram(
                Gdx.files.internal("PrefabsAssets/NemesisProperties/shaders/invdtshdr/vertexShader.vs"),
                Gdx.files.internal("PrefabsAssets/NemesisProperties/shaders/invdtshdr/fragShader.fs"));
